import { wbTreeData } from './weiboViewData';
import {ApiService} from '../utils/request';

//注册命令
export default function getWbHeat(context,vscode) {
  const getWbHeat = vscode.commands.registerCommand('vsplugin.getWbHeat', () => {

    // 传入我们的实例
    const service = new ApiService();
    vscode.window.registerTreeDataProvider("wbHeat", new wbTreeData(service));
  });
