import { MyTreeData } from './treeViewData';
const fs = require('fs');

//打开本地文件夹
export default function openDir(context,vscode) {
  const openDorE = vscode.commands.registerCommand('vsplugin.opendir', () => {
    ...
    vscode.window.showOpenDialog(options).then(result => {
      if (!result) {
        ...
        vscode.window.registerTreeDataProvider("fileList", new MyTreeData(loadDir));
      }
    });
  });
